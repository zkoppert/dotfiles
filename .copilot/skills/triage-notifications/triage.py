...
def apply_todo_mutations(
    ...
    for entry in mutations.route_existing_q2:
        ...
        if not terminal and section in {"prioritized.q1_do_first", "prioritized.q2_schedule"}:
            continue
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--backfill",
        action="store_true",
        help="Dry-run preview: backfill current notifications and tracker rows into a fresh preview ledger without mutating GitHub or todo.yml.",
    )
    parser.add_argument(
        "--backfill-ledger",
        type=Path,
        default=None,
        help="Write the dry-run backfill preview ledger to this fresh path.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
