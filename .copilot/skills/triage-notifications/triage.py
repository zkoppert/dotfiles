...
def _notification_activity_boundary(
    ...
    if not isinstance(tracked, dict):
        return None
    boundary = parse_iso_datetime(tracked.get("terminal_recorded_at"))
    if boundary is None and tracked.get("marked_done"):
...
def build_done_archive_entry_from_tracked(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    Used by the stale notification collector when it drops an inbox/quadrant
...
def _mark_local_notification_done(data: dict[str, Any], delta: MarkDoneDelta) -> bool:
    for item in _iter_todo_items(data):
        ...
        if not (
            ...
            changed = True
        if delta.terminal_recorded_at and notif.get("terminal_recorded_at") != delta.terminal_recorded_at:
            notif["terminal_recorded_at"] = delta.terminal_recorded_at
            changed = True
        if (
...
def reconcile_stats_from_applied(
    ...
    stats.archived_to_done = applied["added_done"]
...
def _stale_notification_prune_delta(
    entry: dict[str, Any], *, section: str
) -> PruneDelta | None:
    """Build a prune delta when a tracked GitHub subject is stale."""
    ...
    if parsed is None:
        return None
    action, stale_reason = check_subject_stale(parsed)
...
def stale_notification_prune_deltas(data: dict[str, Any]) -> list[PruneDelta]:
    """Return stale notification deltas for inbox and quadrant entries."""
    prunes: list[PruneDelta] = []
    for section, items in _active_notification_buckets(data):
        for entry in items:
            if not isinstance(entry, dict):
                continue
            delta = _stale_notification_prune_delta(entry, section=section)
...
def collect_stale_notification_prunes(
    ...
    *,
    ...
    logger.debug("pruner planned %d stale notification prune(s)", len(prunes))
    return prunes
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--backfill",
        action="store_true",
        help="Dry-run preview: backfill a temporary ledger from current notifications and tracker rows without mutating GitHub or todo.yml.",
    )
    parser.add_argument(
        "--backfill-ledger",
        type=Path,
        default=None,
        help="Write the dry-run backfill preview ledger to this path instead of a throwaway temp file.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def preview_backfill_ledger(args: argparse.Namespace) -> TriageStats:
    ...
    if preview_ledger_path is None:
        preview_ledger_path = Path(tempfile.mkdtemp(prefix="notification-backfill-")) / "ledger.sqlite"
    ...
    try:
        ...
        _record_worker_health_snapshot(ledger=ledger, worker="notification-triage", stats=stats)
        return stats
    ...
    for notif in notifications:
        ...
        _ledger_capture(
            ...
            thread_id=thread_id or None,
            canonical_artifact=canonical_url,
            classification="policy_drop" if terminal_disposition else "actionable",
...
def run(args: argparse.Namespace) -> TriageStats:
    ...
    if has_todo_mutations:
        if args.dry_run:
            try:
                applied = preview_todo_mutations(args.todo_file, mutations)
            except (
                OSError,
                FileNotFoundError,
                yaml.YAMLError,
                ...
                stats.errors.append(f"failed to preview todo updates: {exc}")
                _record_worker_health_snapshot(ledger=ledger, worker="notification-triage", stats=stats)
                return stats
        else:
            try:
