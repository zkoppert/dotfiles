...
- **`babysit-prs`** - shepherds my own recently-active open PRs so I stop hand-watching CI. It lives in its own public repo [`zkoppert/babysit-prs`](https://github.com/zkoppert/babysit-prs) (cloned at `~/repos/babysit-prs`, symlinked into `~/.copilot/skills/babysit-prs`); dotfiles keeps only my personal launchd plist (`LaunchAgents/com.zkoppert.babysit-prs.plist`) and the `bin/babysit-prs` wrapper, which points at the clone. It re-runs failed **required** checks once per head commit (a flaky-test retry), updates cleanly-behind branches when the base requires up-to-date branches, and sends one clickable macOS notification **per PR** (no digest) when a PR needs a human: merge conflicts, changes requested, a new human review comment (including inline review-thread replies; bots and the Copilot reviewer are excluded), a failed branch update, a required check still failing after the re-run, or a non-draft PR that is green and ready to merge. Each notification's title is `owner/repo#N`, the subtitle names the reasons, and the body is the PR url (clickable to open via `terminal-notifier`, copyable via the `osascript` fallback). A per-PR state signature in `~/Library/Logs/babysit-prs-state.json` means I am pinged only when a PR's persistent state changes (new comments are de-duped separately via `last_activity`). Guardrails: only required checks are re-run, and a branch is only updated when the base is strict and cleanly behind; anything ambiguous is left for me. The tool defaults to all my open PRs, so my plist passes `--owner github --owner github-community-projects --owner zkoppert` plus `--skip-repo zkoppert/inbox-outdated-branch-check` (the behind-base fixture); the `--active-days` window (default 14) keeps the scan fast. Preview with `python3 ~/repos/babysit-prs/babysit_prs.py --dry-run --verbose`. Clickable notifications use `terminal-notifier` (`brew install terminal-notifier`).
