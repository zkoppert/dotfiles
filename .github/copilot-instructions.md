...
- **`gh-guard`** (`~/repos/dotfiles/bin/gh-guard`, linked at `~/.local/bin/gh`) - intercepts two gh subcommands:
  ...
  - `gh pr create`: blocks PR creation unless BOTH (a) a synthesized multi-model review (>=200 bytes) exists at `<repo>/.git/copilot-pr-review/<branch>.md` (with `%` and `/` in the branch name percent-encoded as `%25` and `%2F`, so `feat/foo` becomes `feat%2Ffoo.md`), AND (b) `ZACK_CONFIRMED_PR_CREATE=1` is set in the invocation environment. This enforces the multi-model Code Review Workflow as a gate, not a guideline. To pass this gate from an agent session: run the workflow, write the synthesis to the marker path, then prefix the `gh pr create` call with `ZACK_CONFIRMED_PR_CREATE=1`.
