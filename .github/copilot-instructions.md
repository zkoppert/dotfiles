...
4. **Commit, review test quality, then multi-model review the code.** Commit the drafted tree so the checker and HEAD-pinned marker review the actual diff, run the `test-quality` skill, include its `## Test quality` section in the multi-model Code Review Workflow synthesis, then run the multi-model review as a convergence loop. After each fix batch, re-run the panel on the updated diff until a round yields no new verified findings, capped at 3 rounds. Record the final clean round: `<code review> | pr-marker write code-review --models a,b,c --convergence-rounds N -`. If I commit more after the review, I re-run this step to re-pin it to the final commit before opening the PR.
...
10. **Request and monitor the Copilot review.** Immediately after creating the draft PR, add Copilot as a requested reviewer through GitHub's reviewer request mechanism. Never request the review by mentioning Copilot in a comment. Watch the draft PR for new review feedback:
    - For Copilot feedback, verify each comment against the code and runtime behavior. Address verified findings, run the appropriate checks, commit and push the fix, reply to the original comment, and resolve the thread when fully addressed. Do not act on inaccurate feedback.
...
- **Handle Copilot review feedback autonomously**: Verify each Copilot comment before acting. For accurate feedback, prepare and validate the fix, commit and push it, reply to the original comment, and resolve the thread when fully addressed. Do not act on inaccurate feedback.
