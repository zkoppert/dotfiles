...
4. **Multi-model review the code.** Run the multi-model Code Review Workflow against the diff, address verified findings, and record the synthesis: `<code review> | pr-marker write code-review --models a,b,c -`. This marker is HEAD-pinned, so if I commit more after it (for example, addressing findings in steps 5 to 7), I re-run this step to re-pin it to the final commit before opening the PR.
5. **Test in a Codespace.** Spin up a Codespace and run the **full** test suite plus the CI-equivalent checks (lint, type-check, build), never just the affected tests, via `pr-marker run-tests --cmd '<command>' [--cmd ...]`, which writes the machine-produced, HEAD-pinned `tests` marker only when every command exits 0. Slowness is never a reason to narrow the suite (hours is fine); only genuine infeasibility, recorded via `--narrow`. Never rely on pushing to CI to find out, and never open a draft PR that then fails CI (see the Harnesses section and my codespace-testing habits).
...
- **Test coverage gaps** - when the diff adds a new function, branch, or code path, check whether a corresponding test exists. Flag untested branches explicitly, especially in repos that enforce a coverage threshold (check `.coveragerc`, `pyproject.toml`, or the Makefile for `--cov-fail-under`). A migration or refactor that adds an `if ghe:` branch with no test for that branch is a reportable gap, not a nitpick. This is high-signal for me.
