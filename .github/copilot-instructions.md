...
- **`gh-guard`** (`~/repos/dotfiles/bin/gh-guard`, linked at `~/.local/bin/gh`) - intercepts three gh subcommands:
  ...
  - `gh pr create`: blocks PR creation unless BOTH (a) a synthesized multi-model review (>=200 bytes) exists at `<git-dir>/copilot-pr-review/<branch>.md` (with `%` and `/` in the branch name percent-encoded as `%25` and `%2F`, so `feat/foo` becomes `feat%2Ffoo.md`), AND (b) `ZACK_CONFIRMED_PR_CREATE=1` is set in the invocation environment. `<git-dir>` is the output of `git rev-parse --absolute-git-dir`, which resolves to `<repo>/.git` for plain checkouts and to `<main>/.git/worktrees/<name>/` for git worktrees (where `<repo>/.git` is a gitlink file, not a directory). The PR body (from `--body`, `--body=`, `--body-file <path>`, or `--body-file=<path>`) is also piped through the `validate-style` lint and the create is blocked on any hard-rule violation. This enforces the multi-model Code Review Workflow as a gate, not a guideline. To pass this gate from an agent session: run the workflow, write the synthesis to the marker path (resolve `<git-dir>` with `git rev-parse --absolute-git-dir` so worktrees work), then prefix the `gh pr create` call with `ZACK_CONFIRMED_PR_CREATE=1`.
