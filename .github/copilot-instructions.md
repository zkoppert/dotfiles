...
3. **Draft the changes locally.** Implement against the reviewed plan.
4. **Review test quality, then multi-model review the code.** Run the `test-quality` skill against the diff, include its `## Test quality` section in the multi-model Code Review Workflow synthesis, address verified findings, and record the synthesis: `<code review> | pr-marker write code-review --models a,b,c -`. This marker is HEAD-pinned, so if I commit more after it (for example, addressing findings in steps 5 to 7), I re-run this step to re-pin it to the final commit before opening the PR.
5. **Test in a Codespace.** Commit the reviewed tree, spin up a Codespace, and run the **full** test suite plus the CI-equivalent checks (lint, type-check, build), never just the affected tests, via `pr-marker run-tests --cmd '<command>' [--cmd ...]`, which runs the test-quality evidence preflight and writes the machine-produced, HEAD-pinned `tests` marker only when every command exits 0 and the repository remains clean. Slowness is never a reason to narrow the suite (hours is fine); only genuine infeasibility, recorded via `--narrow`. Never rely on pushing to CI to find out, and never open a draft PR that then fails CI (see the Harnesses section and my codespace-testing habits).
