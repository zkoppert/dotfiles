...
- **`gh-guard`** (`~/repos/dotfiles/bin/gh-guard`, linked at `~/.local/bin/gh`) - intercepts three gh subcommands:
  ...
  - `gh pr create`: blocks PR creation unless ALL of the following hold, matching my staged change workflow: four marker files exist in the per-branch directory `<git-dir>/copilot-pr-review/<branch>/` (a plan review at `plan.md`, a code review at `code-review.md`, a demo + before/after capture at `demo.md`, and a PR-description review at `pr-review.md`), AND `ZACK_CONFIRMED_PR_CREATE=1` is set in the invocation environment. The `<branch>` directory name is percent-encoded (`%` and `/` become `%25` and `%2F`, so `feat/foo` becomes `feat%2Ffoo`), and `<git-dir>` is the output of `git rev-parse --absolute-git-dir` (which resolves to `<repo>/.git` for plain checkouts and `<main>/.git/worktrees/<name>/` for worktrees). Giving each branch its own directory avoids collisions between, e.g., branch `x`'s plan marker and branch `x.plan`'s code-review marker. Each marker has a byte floor (code review >=200, plan/demo/PR-description >=120) so a bare `touch` never satisfies it. The three review markers (plan, code-review, pr-review) must also declare at least 3 distinct models in a `<!-- reviewed-by-models: a, b, c -->` header (written via `pr-marker --models a,b,c`), so a lightweight single-pass self-review cannot satisfy a gate meant to require a multi-model review; the gate reports such a marker as `FEW-MODELS`. The demo marker is exempt. The code-review marker is also HEAD-pinned: it records the reviewed commit on its first line and is rejected as STALE once HEAD moves past it, so a late commit forces a fresh code review before the PR can open (rewrite it with `pr-marker write code-review --models a,b,c -` to re-pin to the current HEAD). The PR body (from `--body`, `--body=`, `--body-file <path>`, or `--body-file=<path>`) is also piped through the `validate-style` lint and the create is blocked on any hard-rule violation. Write the markers with the `pr-marker` helper (below): `<synthesized content> | pr-marker write <plan|code-review|demo|pr-review> [--models a,b,c] -` (the `--models` flag is required for the three review kinds). To pass this gate from an agent session, write all four markers, then prefix the `gh pr create` call with `ZACK_CONFIRMED_PR_CREATE=1`.
...
- **`pr-marker`** (`~/repos/dotfiles/bin/pr-marker`, linked at `~/.local/bin/pr-marker`) - writes and inspects the four per-branch PR-readiness markers that the `gh pr create` gate checks. It is the single source of truth for the marker path (git-dir resolution plus branch encoding), so I never hand-build the path and land a marker where the gate cannot see it. Usage: `pr-marker status` (show all four and whether each passes), `pr-marker check` (exit non-zero if any is missing, too small, stale, or under-reviewed), `pr-marker write <kind> [file|-] [--models a,b,c]` (write a marker from a file or stdin, refusing content below the floor), `pr-marker path <kind>`, and `pr-marker artifacts-dir <kind>`. Kinds are `plan`, `code-review`, `demo`, `pr-review`. The three review kinds (`plan`, `code-review`, `pr-review`) require `--models` naming at least 3 distinct models, recorded in a `reviewed-by-models` header; the demo kind is exempt. The `code-review` kind is HEAD-pinned: `write` stamps the current commit onto it, and `status`/`check`/the gate report it STALE once HEAD advances. `bin/test_pr_marker.py` pins the branch encoding, byte floors, filenames, pin flags, and the 3-model requirement in gh-guard to `pr-marker`'s `KINDS`.
...
5. **Test in a Codespace.** Spin up a Codespace and run the affected tests, lint, and type checks there (see the Harnesses section and my codespace-testing habits); never rely on pushing to CI to find out.
...
9. **Push and open a draft PR.** After my go-ahead, push the branch and open a draft PR with `ZACK_CONFIRMED_PR_CREATE=1 gh pr create ...` (all four markers must already exist). Assign me, and keep it a draft until I explicitly ask to mark it ready.
