...
Currently installed:
- **`gh-axi`** - use `npx -y gh-axi` as the default interface for every GitHub operation, including issues, pull requests, reviews, workflow runs, releases, repositories, labels, gists, Projects, Actions secrets and variables, search, and raw API calls. Always invoke the `gh-axi` skill first, consult the CLI's current `--help` output for syntax, and do not use GitHub MCP tools. Fall back to the wrapped `gh` CLI only when `gh-axi` does not support the required operation.
...
- **`nux-fr-handoff`** - runs every Friday at noon via launchd, finds the current NUX on-call handoff issue assigned to me, synthesizes every Copilot session active since Monday, refreshes linked GitHub artifact states, creates and verifies a secret gist, and sends a clickable macOS notification for review. The Copilot subprocess can only use `session_store_sql`; Python owns the intentional gist write, and the tool never posts the issue comment automatically. Run `nux-fr-handoff --dry-run --verbose` to preview issue discovery or `nux-fr-handoff --force` to regenerate the current week's draft.
- **`record-demo`** - after code changes are reviewed and tested in a Codespace, and before drafting the PR, drives Playwright against a Codespace-forwarded app to capture before/after images (and an optional recorded walkthrough), saves them in a per-branch artifacts layout, and writes the `demo` marker that gh-guard's `gh pr create` gate requires. For a change with no visual surface, it records an explicit `N/A` justification plus an alternative visual aid instead. Scaffold and verify with `python3 ~/.copilot/skills/record-demo/scaffold.py init|na|check`; the marker is written through `pr-marker write demo`. This is the "record a demo and capture before/after" step of my staged change workflow (below).
- **`validate-pr-with-codespace`** - validates an already-open PR from a fresh Codespace so local state cannot hide setup, test, or integration failures. Use it after opening a PR when clean-environment validation would add confidence. It does not replace `record-demo` and does not satisfy the pre-PR `demo` marker.
- **`session-portability`** - packages general session context for handoffs between local work, Codespaces, and other Copilot sessions. Use it for broad development handoffs; keep specialized workflows such as `nux-fr-handoff` for their narrower triggers and outputs.
- **`cleanup-worktrees`** - safely finds and removes stale Git worktrees after their branches or PRs are finished. Use it for worktree maintenance rather than ad hoc recursive cleanup, and preserve any worktree with uncommitted changes.
- **`remediate-accessibility-audit`** - validates and remediates accessibility audit findings. Use it for accessibility work; do not route accessibility tasks to the observability plugin.
- **`test-quality`** - runs whenever authoring or reviewing tests, changing coverage thresholds, or finalizing production behavior. It checks for source changes without persistent tests, coverage-shaped test artifacts, shallow assertions, missing boundaries, incorrect test ownership, and mocks that do not match a verified real contract. Run `python3 ~/.copilot/skills/test-quality/check.py [--base <ref>]` before the multi-model code review and before `pr-marker run-tests`; the checker enforces only a minimum evidence floor, while the skill's behavior matrix and no-op check judge whether the tests are meaningful.
- **`babysit-prs`** - shepherds the open PRs I authored or that are assigned to me, so I stop hand-watching CI. It lives in its own public repo [`zkoppert/babysit-prs`](https://github.com/zkoppert/babysit-prs) (cloned at `~/repos/babysit-prs`, symlinked into `~/.copilot/skills/babysit-prs`); dotfiles keeps only my personal launchd plist (`LaunchAgents/com.zkoppert.babysit-prs.plist`) and the `bin/babysit-prs` wrapper, which points at the clone. Each run scans the union of PRs I **authored** and PRs **assigned to me**, deduplicated (`gh search prs --author=@me` and `--assignee=@me`). On PRs I authored it auto-acts with guardrails: it re-runs failed **required** checks once per head commit (a flaky-test retry) and updates cleanly-behind branches when the base requires up-to-date branches. PRs I am only assigned to are alert-only, since they are not my branch to mutate. It sends one clickable macOS notification **per PR** (no digest) when a PR needs a human: merge conflicts, changes requested, a new review comment (including inline review-thread replies; the Copilot code reviewer counts, since its comments are actionable, while noisy bots like Dependabot and github-actions stay excluded), a failed branch update, failing CI, a non-draft PR that is green and ready to merge, or an authored PR that has sat ready for review with no updates for `--nudge-weekdays` weekdays (default 3, weekends excluded) and needs me to nudge the reviewers. The nudge fires only when the ball is on the reviewers, so it stays quiet when the PR has conflicts, requested changes, failing CI, an approval, or a scheduled auto-action; the `--active-days` scan window is widened automatically so a configured nudge is never silently unreachable. Each notification's title is `owner/repo#N`, the subtitle names the reasons, and the body is the PR url (clickable to open via `terminal-notifier`, copyable via the `osascript` fallback). A per-PR state signature in `~/Library/Logs/babysit-prs-state.json` means I am pinged only when a PR's persistent state changes; new comments and nudges are event-like and de-duped separately (by `last_activity` and the `updatedAt` a nudge last fired at). The tool is organized as focused modules (`constants`, `ghapi`, `checks`, `activity`, `classify`, `dedup`, `effects`, `runner`) behind the `babysit_prs.py` entry point. My plist passes `--owner github --owner github-community-projects --owner zkoppert` plus `--skip-repo zkoppert/inbox-outdated-branch-check` (the behind-base fixture); the `--active-days` window (default 14) keeps the scan fast. Preview with `python3 ~/repos/babysit-prs/babysit_prs.py --dry-run --verbose`. Clickable notifications use `terminal-notifier` (`brew install terminal-notifier`).

### Copilot Plugins

- **`gho11y`** - GitHub observability workflows for investigating production signals and operational behavior. Use it for observability work, not accessibility audits.

### Wrappers (PATH shims, not skills)

- **`gh-guard`** (`~/repos/dotfiles/bin/gh-guard`, linked at `~/.local/bin/gh`) - intercepts three gh subcommands:
  - `gh pr ready`: requires confirmation before marking a draft PR as ready for review (matches the "never mark draft PRs ready without explicit ask" rule). `gh pr ready --undo` is allowed to pass through.
  - `gh pr create`: blocks PR creation unless ALL of the following hold, matching my staged change workflow: five marker files exist in the per-branch directory `<git-dir>/copilot-pr-review/<branch>/` (a plan review at `plan.md`, a code review at `code-review.md`, a demo + before/after capture at `demo.md`, a PR-description review at `pr-review.md`, and a machine-produced full-test-suite run at `tests.md`), AND `ZACK_CONFIRMED_PR_CREATE=1` is set in the invocation environment. The `<branch>` directory name is percent-encoded (`%` and `/` become `%25` and `%2F`, so `feat/foo` becomes `feat%2Ffoo`), and `<git-dir>` is the output of `git rev-parse --absolute-git-dir` (which resolves to `<repo>/.git` for plain checkouts and `<main>/.git/worktrees/<name>/` for worktrees). Giving each branch its own directory avoids collisions between, e.g., branch `x`'s plan marker and branch `x.plan`'s code-review marker. Each marker has a byte floor (code review >=200, plan/demo/PR-description/tests >=120) so a bare `touch` never satisfies it. The three review markers (plan, code-review, pr-review) must also declare at least 3 distinct models in a `<!-- reviewed-by-models: a, b, c -->` header (written via `pr-marker --models a,b,c`), so a lightweight single-pass self-review cannot satisfy a gate meant to require a multi-model review; the gate reports such a marker as `FEW-MODELS`. The code-review marker must additionally declare its final clean convergence round through `--convergence-rounds 1|2|3`; both implementations reject missing or invalid metadata. The demo and tests markers are model-exempt. The code-review and tests markers are HEAD-pinned: they record the reviewed/tested commit on the first line and are rejected as STALE once HEAD moves past it, so a late commit forces a fresh code review and a fresh test run before the PR can open (re-run and rewrite them). The `tests` marker is machine-produced: `pr-marker run-tests --cmd '<command>'` runs the given check commands and writes it only if every command exits 0, so a bare touch or a self-reported note does not pass (a deliberate hand-forge of the header literal is out of scope: the gate guards against my own accidental shortcuts, not self-forgery by the repo owner); slowness is never a reason to narrow the suite, only genuine infeasibility (recorded via `--narrow`). The PR body (from `--body`, `--body=`, `--body-file <path>`, or `--body-file=<path>`) is also piped through the `validate-style` lint and the create is blocked on any hard-rule violation. Write the review/demo markers with `<synthesized content> | pr-marker write <plan|code-review|demo|pr-review> [--models a,b,c] [--convergence-rounds N] -` (the `--models` flag is required for the three review kinds, and `--convergence-rounds` is required for code-review) and the tests marker with `pr-marker run-tests`. To pass this gate from an agent session, write all five markers, then prefix the `gh pr create` call with `ZACK_CONFIRMED_PR_CREATE=1`.
  - `gh pr edit`: when `--body` or `--body-file` is provided, the new body content is piped through the same `validate-style` lint and the edit is blocked on any hard-rule violation. Edits that touch only title, labels, reviewers, base, milestone, or other non-body fields pass through silently. This closes the loop where post-create body edits would otherwise bypass the `pr create` lint.

- **`pr-marker`** (`~/repos/dotfiles/bin/pr-marker`, linked at `~/.local/bin/pr-marker`) - writes and inspects the five per-branch PR-readiness markers that the `gh pr create` gate checks. It is the single source of truth for the marker path (git-dir resolution plus branch encoding), so I never hand-build the path and land a marker where the gate cannot see it. Usage: `pr-marker status` (show all five and whether each passes), `pr-marker check` (exit non-zero if any is missing, too small, stale, or under-reviewed), `pr-marker write <kind> [file|-] [--models a,b,c] [--convergence-rounds N]` (write a marker from a file or stdin, refusing content below the floor), `pr-marker run-tests --cmd '<command>' [--cmd ...] [--base-ref <ref>] [--no-test-change-reason <reason>] [--narrow reason]` (run the test-quality evidence preflight and check commands, then write the machine-produced `tests` marker only if the repository stays clean and every command exits 0), `pr-marker path <kind>`, and `pr-marker artifacts-dir <kind>`. Kinds are `plan`, `code-review`, `demo`, `pr-review`, `tests`. The three review kinds (`plan`, `code-review`, `pr-review`) require `--models` naming at least 3 distinct models, recorded in a `reviewed-by-models` header; `code-review` also requires a final clean convergence round from 1 through 3. `demo` and `tests` are model-exempt. The `code-review` and `tests` kinds are HEAD-pinned: `write`/`run-tests` stamps the current commit onto them, and `status`/`check`/the gate report them STALE once HEAD advances. `run-tests` refuses staged, unstaged, or untracked non-ignored files because a HEAD-pinned marker must certify exactly the committed tree. `bin/test_pr_marker.py` pins the branch encoding, byte floors, filenames, pin flags, model requirement, and convergence requirement in gh-guard to `pr-marker`'s `KINDS`.
...
## My Standard Change Workflow

Every non-trivial change follows these staged steps. The gates (`gh-guard` + `pr-marker`) and skills exist to make this the path of least resistance, not to add friction. `pr-marker status` shows which markers are still outstanding at any point.

1. **Plan.** Write the plan for the change.
2. **Multi-model review the plan.** Run the multi-model Code Review Workflow (below) against the plan, address findings, and record the synthesis: `<plan review> | pr-marker write plan --models a,b,c -`.
3. **Draft the changes locally.** Implement against the reviewed plan and run focused tests while iterating.
4. **Commit, review test quality, then multi-model review the code.** Commit the drafted tree so the checker and HEAD-pinned marker review the actual diff, run the `test-quality` skill, include its `## Test quality` section in the multi-model Code Review Workflow synthesis, then run the multi-model review as a convergence loop. Every certification round must use newly launched, session-free review agents that did not author the code or participate in the fix turn. A reviewer must never certify its own recommendation, and prior fix summaries are claims to verify rather than evidence. After each fix batch, re-run the panel on the updated diff until a round yields no new verified findings, capped at 3 rounds. Record the final clean round: `<code review> | pr-marker write code-review --models a,b,c --convergence-rounds N -`. If I commit more after the review, I re-run this step to re-pin it to the final commit before opening the PR.
5. **Test in a Codespace.** Spin up a Codespace and run the **full** test suite plus the CI-equivalent checks (lint, type-check, build), never just the affected tests, via `pr-marker run-tests --cmd '<command>' [--cmd ...]`, which runs the test-quality evidence preflight and writes the machine-produced, HEAD-pinned `tests` marker only when every command exits 0 and the repository remains clean. Slowness is never a reason to narrow the suite (hours is fine); only genuine infeasibility, recorded via `--narrow`. Never rely on pushing to CI to find out, and never open a draft PR that then fails CI (see the Harnesses section and my codespace-testing habits).
6. **Record a demo and capture before/after.** Use the `record-demo` skill (Codespace + Playwright) to capture before/after images and an optional walkthrough, then write the `demo` marker. For a change with no visual surface, record an explicit `N/A` justification plus an alternative visual aid.
7. **Draft the PR description, then multi-model review it.** Write the PR body (problem-first, first person, using the repo template and the before/after block from step 6), run the multi-model review against it, and record the synthesis: `<PR-description review> | pr-marker write pr-review --models a,b,c -`.
8. **My human review of it all.** I review the plan, the code, the demo, and the PR description before anything is pushed. Wait for my go-ahead.
9. **Push and open a draft PR.** After my go-ahead, push the branch and open a draft PR with `ZACK_CONFIRMED_PR_CREATE=1 gh pr create ...` (all five markers must already exist). Assign me, and keep it a draft until I explicitly ask to mark it ready.
10. **Request and monitor the Copilot review.** Immediately after creating the draft PR, add Copilot as a requested reviewer through GitHub's reviewer request mechanism. Never request the review by mentioning Copilot in a comment. Watch the draft PR for new review feedback:
    - For Copilot feedback, verify each comment against the code and runtime behavior. Address verified findings and commit the fix locally. Before pushing, run the `test-quality` skill, run the full multi-model code-review convergence loop with newly launched session-free reviewers, run the full Codespace test suite and CI-equivalent checks through `pr-marker run-tests`, and refresh the HEAD-pinned `code-review` and `tests` markers for the fix commit. Push only after the new HEAD is fully certified, then reply to the original comment and resolve the thread when fully addressed. Do not act on inaccurate feedback.
    - For human feedback, verify each comment and prepare and validate the fix locally. Do not reply, resolve the thread, or push the fix commit until I explicitly approve those actions.
...
- **Coverage is diagnostic, not the goal.** Design scenarios from requirements first, then use coverage to find forgotten code. Never create tests named or documented by source line solely to increase coverage. For every important test, apply the no-op check: it must fail if the protected behavior is removed, inverted, or returns the wrong value.
- **Commit deterministic regression tests.** An ad hoc validation script or tests-marker command does not replace a persistent repository test for behavior that can be tested deterministically. If committed tests are genuinely infeasible, record a specific reason with `--no-test-change-reason`.
...
- **Acknowledge harness gaps explicitly.** When a harness genuinely isn't possible (no dev server, no browser, no testable output, or the operation is destructive/irreversible and can't be safely rehearsed), say so plainly and ask me how I want to validate. Don't quietly skip the validation step and hope.
...
- **Request Copilot review after draft creation**: Immediately after opening a draft PR, add Copilot as a requested reviewer through GitHub's reviewer request mechanism. Never request a Copilot review by posting `@copilot review` or mentioning Copilot in a comment.
...
- **Handle Copilot review feedback autonomously**: Verify each Copilot comment before acting. For accurate feedback, prepare and commit the fix locally, then complete the post-PR fix revalidation gate before pushing, replying, or resolving the thread. Do not act on inaccurate feedback.
- **Hold human review feedback for my approval**: Verify each human comment and prepare and validate the fix locally, but never reply, resolve the thread, or push the fix commit until I explicitly approve those actions.
- **Revalidate every post-PR fix before pushing**: Any commit created after the draft PR opens invalidates the earlier certification, regardless of whether the fix came from Copilot, a human reviewer, CI, or self-review. Before pushing the fix, run `test-quality`, run a fresh multi-model code-review convergence loop whose reviewers did not author or fix the code, run the full Codespace test suite and CI-equivalent checks through `pr-marker run-tests`, and rewrite the HEAD-pinned `code-review` and `tests` markers for the exact fix commit. Never push a post-PR code change while either marker is stale.
...
#### Convergence loop (code gate only)
Run the **code diff** gate as a convergence loop, not a single pass:
- After addressing a batch of verified findings, re-run the full multi-model panel on the updated diff. Repeat until a round surfaces no new verified findings.
- Every initial review and re-review must use newly launched, session-free reviewers. Do not resume a reviewer's prior conversation, reuse a fixing agent as a reviewer, or let a reviewer certify code written to satisfy its own recommendation. Give the new panel the updated diff and relevant requirements, but treat prior findings, fix summaries, and tests from the fix turn as claims that require independent verification.
- **Cap at 3 rounds.** Stop when a round is clean. If the third round still surfaces a verified blocking finding, fix it, say the cap was reached, and let me decide whether to continue.
- Record the final clean round with `pr-marker write code-review --models a,b,c --convergence-rounds N -`. Both `pr-marker` and `gh-guard` reject a code-review marker without valid final-clean-round metadata.
- The **plan** and **PR-description** gates stay single-pass unless I ask otherwise.
...
- **Performance footprint** - judge the change's overall cost, not just its correctness: added database queries or N+1s, extra allocations or memory growth, larger response or client-bundle size, new synchronous or blocking calls, and added latency across the whole request, render, or job. For a change on a request path or inside a large loop, estimate the added cost (queries, calls, bytes) and say whether it warrants a measurement or a benchmark before shipping. This is the holistic companion to the narrower "New hot-path cost" check below and to "Design for 10x".
- **New hot-path cost** - when the diff adds per-request work on a hot path (a new feature-flag or config lookup, an extra query, a cache or network call, an N+1), confirm it reuses the batching, caching, or preloading the surrounding path already relies on. Such a regression is invisible to functional tests, so it has to be caught in review; for example, a feature-flag read added to a private application's controller action but not to that action's feature preloader costs a separate cache call every request. Name the batch or cache to reuse.
...
- **Comment minimalism (codified, not a nitpick)** - flag comments that restate what the code or the method/variable name already says, or that explain "what" instead of "why". This enforces the "Comments explain why, not what" rule from Code Style, so surface redundant or over-verbose comments for removal even though generic comment style is otherwise suppressed. Apply it when authoring too: strip redundant comments before committing.
...
- **Removed safety nets** - when a refactor deletes a defensive guard (a `None`/empty check, validation, `try/except`, `if not connection:` fallback, or an explicit `raise`), do not assume the new library or path replaces it. Ask explicitly: what happens now on that failure case? Flag any removed guard whose failure mode is no longer handled, naming the old check and the new unguarded path.
- **Library-migration redundancy** - when a PR swaps libraries, check whether scaffolding the old library needed is now dead weight the new one provides natively: custom retry/backoff, timeout wrappers, session monkey-patching, manual JWT/token construction, pagination helpers. Flag custom code that duplicates a built-in default and recommend dropping it.
- **Redundant round-trips** - flag values decomposed and then immediately recomposed (e.g. `owner, name = x.split("/")` followed by `get_repo(f"{owner}/{name}")`) when the split can drop or mangle data the recompose assumes intact (`split("/")` vs `split("/", 1)`, lost trailing slashes). Skip it when the transform is purely cosmetic; report it as a low-priority simplification when collapsing removes a real failure surface.
- **Check docstring/comment accuracy** - verify that docstrings, comments, and commit messages accurately describe what the code actually does. Flag cases where stated behavior differs from implemented behavior. Also verify type hints and function signatures match how the function is actually called (e.g., a parameter typed `str` but always passed an `int`).
- **Test quality pass** - run the `test-quality` skill before synthesizing the code review. Map each changed behavior to a specific test and observable assertion, then check exact boundaries, malformed/empty input, partial responses, timeouts, feature flag disabled/enabled, and the final page/batch/chunk where relevant. Flag tests that only assert mock calls, constructed arguments, non-null/type checks, or framework behavior instead of the owning abstraction's result. Coverage is a clue, not proof that these scenarios are specified.
- **Persistent test coverage gaps** - when the diff adds a new function, branch, or code path, check whether a committed regression test exists. A migration or refactor that adds an `if ghe:` branch with no test is a reportable gap, not a nitpick, even when an ad hoc harness or coverage report is green. Preserve existing feature-flag-off and failure-path regression tests during refactors.
- **Dependency hygiene** - when a PR adds, removes, or swaps a library, verify the dependency manifests and lockfiles agree with actual imports. Flag dependencies left in `pyproject.toml`/`requirements.txt`/lockfile that are no longer imported anywhere, and a dependency someone proposes removing that is in fact still imported (name the file and line). Surface the "is this still needed?" question with the answer, not just silence.
- **Surface high-value pre-existing issues** - tight scoping to the diff is the default, but do not silently drop a verified latent bug, dead code path, or comment/code mismatch you find in a file the PR touches just because the PR did not introduce it. Report it labeled **pre-existing**, with a recommendation to fix-while-here or file a follow-up issue (see "Capture side quests as issues").
...
### Evidence synthesis
For recurring reports, retrospectives, handoffs, and performance feedback:
- Collect source evidence before interpreting it, then extract themes before reading editorial configuration that could bias the synthesis.
- Tag claims with provenance and flag any conclusion supported by only one source.
- Pause at an evidence gate before drafting high-stakes or sensitive output. Confirm that the evidence is sufficient, current, and relevant.
- Structure findings as what happened, why it matters, and the impact.
- Resolve uncertainty when the source can answer it. Disclose the few remaining low-confidence claims directly without weakening ownership of the output.
- Preserve progress, source references, and recovery state so an interrupted session can resume without reconstructing the evidence set.
...
## GitHub CLI & API
- Use **`gh-axi`** instead of GitHub MCP tools for GitHub interactions. Prefer `npx -y gh-axi`, then fall back to the wrapped **`gh` CLI** only for operations `gh-axi` does not support.
