...
2. **Multi-model review the plan.** Run the multi-model Code Review Workflow (below) against the plan, address findings, and record the synthesis: `<plan review> | pr-marker write plan -`.
3. **Draft the changes locally.** Implement against the reviewed plan.
4. **Multi-model review the code.** Run the multi-model Code Review Workflow against the diff, address verified findings, and record the synthesis: `<code review> | pr-marker write code-review -`. This marker is HEAD-pinned, so if I commit more after it (for example, addressing findings in steps 5 to 7), I re-run this step to re-pin it to the final commit before opening the PR.
...
7. **Draft the PR description, then multi-model review it.** Write the PR body (problem-first, first person, using the repo template and the before/after block from step 6), run the multi-model review against it, and record the synthesis: `<PR-description review> | pr-marker write pr-review -`.
