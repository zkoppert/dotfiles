...
- When presenting a list of options and recommending one, explain why it is the recommendation
...
- **`babysit-prs`** - runs every 15 minutes via launchd, scans my recently active PRs, repairs safe CI and branch states, and deploys approved green PRs in explicitly enabled repositories to review environments once per head commit. Review-lab deploys run only from my local Mac because `gh-slack` authentication is unavailable in Codespaces. The scheduled repositories are configured with repeatable `--review-lab-repo OWNER/REPO` flags.
...
4. **Multi-model review the code.** Run the multi-model Code Review Workflow against the diff as a **convergence loop**: after each fix batch, re-run the panel on the updated diff and repeat until a round yields no new verified findings, capped at 3 rounds (see "Convergence loop" under the Code Review Workflow). Address verified findings, and record the final round's synthesis: `<code review> | pr-marker write code-review --models a,b,c -`. This marker is HEAD-pinned, so if I commit more after it (for example, addressing findings in steps 5 to 7), I re-run this step to re-pin it to the final commit before opening the PR.
...
10. **Deploy after approval.** Once an authored PR in an enabled repository is approved and every known required check is green, `babysit-prs` deploys that head commit to its review environment from my local Mac. It uses `review-lab` by default and `preview` for `github/github-ui`. Do not rely on the PR description promise alone.
...
- **Multi-model review at three gates**: Complete the multi-model Code Review Workflow (below) three times across my staged change workflow: against the **plan** (before drafting code), against the **code** diff (before drafting the PR), and against the **PR description** (before opening the PR). The **code** diff gate runs as a convergence loop (re-review after each fix batch until a clean round, capped at 3 rounds); the plan and PR-description gates are single-pass. Record each synthesis with `pr-marker write <plan|code-review|pr-review> -` so the `gh pr create` gate can see it. Address verified findings at each gate, when they are cheapest to fix. See "My Standard Change Workflow" above.
...
- **Deploy approved PRs to review environments**: For repositories configured in `babysit-prs`, treat approval plus green required CI as the review-lab trigger. The local launchd job submits each head commit once with `gh review-lab deploy <PR URL> --target <review-lab|preview> --wait --no-confirm`. If automation reports a deploy failure, fix the local `gh-slack` authentication or deploy manually with the `deploy` shell alias.
...
Run the **code diff** gate as a convergence loop, not a single pass, because one round demonstrably leaves bugs behind:
- After addressing a batch of verified findings, re-run the full multi-model panel on the updated diff. Repeat until a round surfaces no new verified findings (blocking or non-blocking).
- **Cap at 3 rounds.** Stop the moment a round comes back clean; do not loop for the sake of it. The cap bounds cost and avoids endless nitpicking on diminishing returns.
- If the 3rd round still surfaces a verified blocking finding, fix it, then tell me the cap was reached and let me decide whether to continue, rather than looping silently.
- Each re-run is a fresh panel of at least 3 models on the current diff, so re-pin the HEAD-pinned `code-review` marker to the final reviewed commit. Record the last round's synthesis (noting how many rounds ran and that the final round was clean) in the marker.
- The **plan** and **PR-description** gates stay single-pass unless I ask otherwise; convergence is worth the extra rounds only on the code diff, where the blast radius of a missed bug is highest.
