...
4. **Commit, review test quality, then multi-model review the code.** Commit the drafted tree so the checker and HEAD-pinned marker review the actual diff, run the `test-quality` skill, include its `## Test quality` section in the multi-model Code Review Workflow synthesis, address verified findings, and record the synthesis: `<code review> | pr-marker write code-review --models a,b,c -`. If I commit more after the review, I re-run this step to re-pin it to the final commit before opening the PR.
